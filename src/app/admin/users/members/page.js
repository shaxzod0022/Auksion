...
export default function MembersPage() {
  ...
  const handleOpenModal = (user = null) => {
    if (user) {
      ...
      setFormData({
        ...
        password: "",
        newPassword: "",
        region: user.fullAddress?.region || "",
        city: user.fullAddress?.city || "",
        street: user.fullAddress?.street || "",
        houseNumber: user.fullAddress?.houseNumber || ""
      });
    } else {
      setEditingUser(null);
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingUser) {
        // Update
        const { region, city, street, houseNumber, ...restData } = formData;
        const payload = { ...restData, fullAddress: { region, city, street, houseNumber } };
        delete payload.password; // Update handles newPassword
        ...
        const result = await userService.createUser({
          ...restData,
          fullAddress: { region, city, street, houseNumber },
          role: "user",
        });
        if (result.message && result.message.includes("muvaffaqiyatli")) {
          fetchUsers();
                  ...
                  <td>
                    {user.fullAddress ? (
                      <div style={{ fontSize: '0.85rem' }}>
                        {user.fullAddress.region}, {user.fullAddress.city},<br/>
                        <span style={{ fontSize: '0.75rem', color: 'var(--admin-text-muted)' }}>
                          {user.fullAddress.street}, {user.fullAddress.houseNumber}
                        </span>
                      </div>
                    ) : (
                      <div style={{ fontSize: '0.85rem', color: '#ef4444' }}>Manzil yo'q</div>
                    )}
                  </td>
        ...
        <div className="admin-modal-overlay">
          <div className="admin-modal" style={{ maxWidth: '800px' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>Viloyat</label>
                  <input
                    className="admin-input"
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>Tuman/Shahar</label>
                  <input
                    className="admin-input"
                    value={formData.city}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>Ko'cha</label>
                  <input
                    className="admin-input"
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>Uy raqami</label>
                  <input
                    className="admin-input"
              ...
              {!editingUser ? (
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>Yangi Parol (Admin sifatida parolni to'g'ridan-to'g'ri yangilash mumkin)</label>
                  <input
                    type="password"
                    className="admin-input"
