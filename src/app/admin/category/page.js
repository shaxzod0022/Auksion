...
import { Plus, Edit2, Trash2, X, Save, AlertCircle, Loader2, Image as ImageIcon } from "lucide-react";
...
export default function CategoryPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCategory) {
        // Update
        const result = await categoryService.updateCategory(editingCategory._id, data);
          ...
          <table className="admin-table">
            ...
            <tbody>
              {categories.map((cat) => (
                <tr key={cat._id}>
                  <td>
                    <div style={{ width: '50px', height: '50px', borderRadius: '0.5rem', overflow: 'hidden', background: 'var(--admin-bg-secondary)' }}>
                      <img 
                  ...
                  <td style={{ color: 'var(--admin-text-muted)', fontSize: '0.875rem' }}>{cat.slug}</td>
                  <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                      <button onClick={() => handleOpenModal(cat)} className="admin-nav-item" style={{ padding: '0.5rem' }}>
                        <Edit2 size={18} />
                      </button>
                      <button onClick={() => handleDelete(cat._id)} className="admin-nav-item" style={{ padding: '0.5rem', color: '#ef4444' }}>
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
          ...
          <div className="admin-modal">
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>Kategoriya Nomi</label>
                <input
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>Kategoriya Rasmi</label>
                <div 
                ...
                >
                  ...
                  {imagePreview ? (
                    ...
                    <div style={{ color: 'var(--admin-text-muted)' }}>
                      ...
                      <p style={{ fontSize: '0.75rem' }}>PNG, JPG, WEBP formatlari</p>
                    </div>
                  )}
                  {imagePreview && (
