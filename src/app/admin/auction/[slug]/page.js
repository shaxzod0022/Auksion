...
export default function AdminAuctionRoom({ params }) {
  ...
  useEffect(() => {
    const initAdminAuction = async () => {
      try {
        ...
        socketInstance.emit("join_auction", { slug, isAdmin: true, userId: "admin", userName: "Admin" });
  ...
  const handleAdminEnd = () => {
    if (socket && window.confirm("Haqiqatan ham auksionni hoziroq yakunlamoqchimisiz?")) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 text-[#18436E]">
        ...
        <p className="text-xl font-bold uppercase tracking-widest">Admin boshqaruvi ulanmoqda...</p>
      </div>
    );
  }
  ...
  if (tooEarly) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center">
        ...
        <h1 className="text-3xl font-black text-[#18436E] uppercase mb-4">Hali vaqt bor</h1>
        ...
        <Link href="/admin/lots" className="bg-[#18436E] text-white px-8 py-3 rounded-sm font-bold shadow-lg">
          Lotlarga qaytish
        </Link>
      </div>
    );
  ...
  const stepValue = lot ? Math.floor((lot.startPrice * lot.firstStep) / 100) : 0;
      ...
      <header className="bg-[#0f172a] text-white p-4 shadow-xl">
        <div className="max-w-[1600px] mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            <Link href="/admin/lots" className="bg-white/10 p-2 rounded-sm border border-white/20 hover:bg-white/20 transition-all">
              <ShieldAlert size={20} className="text-blue-400" />
            </Link>
            <div>
              <h1 className="text-lg font-black uppercase tracking-tight leading-none">
                {lot?.name} <span className="text-gray-400 text-sm">(ADMIN NAZORATI)</span>
              </h1>
          ...
          <div className="flex items-center gap-3">
            {phase === "waiting" && (
              <div className="bg-blue-600 px-4 py-2 rounded-sm flex items-center gap-2">
                <Clock size={16} />
                <span className="text-xs font-black uppercase">Ishtirokchilar kutilmoqda</span>
              </div>
            )}
            {phase === "prep" && (
              <div className="bg-orange-500 px-4 py-2 rounded-sm flex items-center gap-2">
                <Clock size={16} />
                <span className="text-xs font-black uppercase">TAYYORGARLIK: {formatTime(timeLeft)}</span>
              </div>
            )}
            {phase === "bidding" && (
              <div className="bg-green-600 px-4 py-2 rounded-sm flex items-center gap-2 animate-pulse font-mono">
                <Clock size={16} />
                <span className="text-xs font-black uppercase">SAVDO: {timeLeft}s</span>
              </div>
            )}
            {phase === "ended" && (
              <div className="bg-red-700 px-4 py-2 rounded-sm">
                <span className="text-xs font-black uppercase">Auksion Yakunlangan</span>
              </div>
            )}
            ...
            {phase !== "ended" && phase !== "waiting" && (
              <button
                ...
                className="bg-red-600 text-white px-5 py-2 rounded-sm text-xs font-black uppercase hover:bg-red-700 transition-all shadow-md"
              >
                Yakunlash
              </button>
            )}
      ...
      <main className="flex-1 flex flex-col lg:flex-row max-w-[1600px] mx-auto w-full p-4 gap-4 overflow-hidden">
        ...
        <div className="lg:w-1/4 flex flex-col gap-4">
          ...
          <div className="bg-white p-5 rounded-sm shadow-sm border border-gray-200">
            <p className="text-[10px] text-gray-400 font-black uppercase mb-1">Boshlang'ich narx</p>
            <h3 className="text-xl font-black text-gray-500">{lot?.startPrice?.toLocaleString()} UZS</h3>
          </div>
          <div className="bg-white p-5 rounded-sm shadow-sm border border-gray-200">
            <p className="text-[10px] text-green-600 font-black uppercase mb-1">Joriy narx</p>
            <h3 className="text-3xl font-black text-[#18436E]">{currentPrice?.toLocaleString()} UZS</h3>
          </div>
          <div className="bg-white p-4 rounded-sm shadow-sm border border-gray-100">
            <h4 className="text-xs font-black uppercase text-gray-400 border-b pb-2 mb-3">Lot ma'lumotlari</h4>
            <div className="space-y-2 text-xs font-bold">
              <div className="flex justify-between">
                <span className="text-gray-500">Qadam:</span>
                <span className="text-[#18436E]">{lot?.firstStep}% ({stepValue.toLocaleString()} UZS)</span>
              </div>
              <div className="flex justify-between">
          ...
          {phase === "prep" && (
            <div className="bg-orange-50 p-6 rounded-sm border-2 border-dashed border-orange-300 text-center">
              <p className="text-orange-700 font-black uppercase text-sm mb-2">Tayyorgarlik bosqichi</p>
              <div className="text-5xl font-black text-orange-600">{formatTime(timeLeft)}</div>
            </div>
          )}
          ...
          {phase === "bidding" && (
            <div className="bg-green-50 p-6 rounded-sm border border-green-200 text-center">
              ...
              <p className="text-green-700 font-black uppercase text-sm mb-1">Keyingi qadamgacha</p>
              <div className="text-5xl font-black text-green-600">{timeLeft}s</div>
            </div>
          )}
        ...
        <div className="lg:flex-1 bg-white rounded-sm shadow-sm border border-gray-100 flex flex-col">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-black text-[#18436E] uppercase text-sm">Savdolar Jurnali (Admin View)</h3>
            </div>
            <span className="text-[10px] text-gray-400 uppercase font-bold">Jonli Yangilanmoqda</span>
          </div>
          <div
            ref={scrollRef}
          ...
          >
            {bids.length === 0 ? (
              ...
              bids.map((bid, i) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <span className="text-gray-400 text-xs">[{new Date(bid.time).toLocaleTimeString()}]</span>
                    <span className="font-bold text-gray-700">{bid.userName}</span>
                  </div>
                  <div className="flex items-center gap-6">
                    <span className="text-green-600 font-black">+{stepValue.toLocaleString()}</span>
                    <span className="font-black text-[#18436E]">{bid.amount?.toLocaleString()} UZS</span>
                  </div>
                </div>
              ))
            )}
          ...
          {phase === "ended" && (
            <div className="p-6 bg-[#0f172a] text-white text-center border-t-4 border-yellow-500">
              ...
              <h2 className="text-2xl font-black uppercase mb-2">Auksion Yakunlandi</h2>
              <p className="text-lg font-bold text-yellow-400">{lastBidder?.userName || "G'olib aniqlanmadi"}</p>
              <p className="text-2xl font-black mt-2">{currentPrice?.toLocaleString()} UZS</p>
            </div>
          )}
        </div>
        ...
        <div className="lg:w-1/5 flex flex-col gap-4">
          <div className="bg-white p-4 rounded-sm shadow-sm border border-gray-200">
            ...
            {bids.length === 0 ? (
              <p className="text-xs text-gray-400 italic text-center py-4">Hali hech kim qadam bosmagan</p>
            ) : (
              <div className="space-y-2">
                {Array.from(new Set(bids.map(b => b.userId))).map(uid => {
                  ...
                  return (
                    ...
                    >
                      <CheckCircle2 size={14} className={isWinner ? "text-yellow-500" : "text-green-400"} />
          ...
          <div className="bg-[#0f172a] p-5 rounded-sm text-white">
            ...
            <h4 className="font-black uppercase text-sm mb-2">Savdo Qoidalari</h4>
            <div className="text-[10px] text-gray-400 space-y-1 leading-relaxed">
              <p>• Qadam: {lot?.firstStep}% ({stepValue.toLocaleString()} UZS)</p>
