...
export default function LotsPage() {
  ...
  const handleViewApplicants = async (lot) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://auksion-backend-production.up.railway.app/api/application/lot/${lot._id}`,
        {
  ...
  const handleUpdateAppStatus = async (appId, newStatus) => {
    try {
      const token = sessionStorage.getItem("adminToken");
      const res = await fetch(
        `https://auksion-backend-production.up.railway.app/api/application/${appId}/status`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDeleteApplicant = async (appId) => {
    ...
    try {
      const token = sessionStorage.getItem("adminToken");
      const res = await fetch(
        `https://auksion-backend-production.up.railway.app/api/application/${appId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
